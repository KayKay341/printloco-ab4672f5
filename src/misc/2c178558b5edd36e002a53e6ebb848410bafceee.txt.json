...
/**
 * Tajima DST / Brother PES parser.
 * Enough to draw a faithful preview and count stitches.
 */
function parseEmbroidery(buf: ArrayBuffer, ext: string): ParsedDst {
  if (ext === "dst") return parseDst(buf);
  if (ext === "pes") return parsePes(buf);
  throw new Error("Unsupported format");
}

function parsePes(buf: ArrayBuffer): ParsedDst {
  const u8 = new Uint8Array(buf);
  // PES files contain a PEC block which has the actual stitch data.
  // Find the PEC offset (it's stored as a 4-byte int at offset 8)
  const view = new DataView(buf);
  const pecOffset = view.getUint32(8, true);
  
  if (pecOffset >= u8.length) throw new Error("Invalid PES file");

  // PEC header starts with "LA:" followed by color count and then stitches
  // Stitches in PEC are encoded as deltas.
  const points: ParsedDst["points"] = [{ x: 0, y: 0, jump: false, colorChange: false }];
  let x = 0, y = 0, stitches = 0, colorChanges = 0;

  // Jump to PEC stitch data (usually offset 512 inside PEC block)
  let i = pecOffset + 512;
  while (i + 1 < u8.length) {
    let b1 = u8[i], b2 = u8[i + 1];
    i += 2;

    if (b1 === 0xff && b2 === 0x00) break; // End of pattern

    if (b1 === 0xfe && b2 === 0xb0) {
      colorChanges++;
      points.push({ x: x / 10, y: y / 10, jump: false, colorChange: true });
      continue;
    }

    // Decode PEC delta
    let dx = b1 > 127 ? b1 - 256 : b1;
    let dy = b2 > 127 ? b2 - 256 : b2;
    
    // Check for "Jump" command (0x80 prefix)
    if (b1 === 0x80) {
      ...
      dy = b3 > 127 ? b3 - 256 : b3;
      x += dx; y += dy;
      points.push({ x: x / 10, y: y / 10, jump: true, colorChange: false });
    } else {
      x += dx; y += dy;
      stitches++;
      points.push({ x: x / 10, y: y / 10, jump: false, colorChange: false });
    }
  }

  return {
    stitchCount: stitches,
    widthMm: 100, // Heuristic for now
    heightMm: 100,
    colorChanges,
    points,
  };
...
export default function EmbroideryPreview({ file }: Props) {
  ...
  useEffect(() => {
    ...
    if (ext !== "dst" && ext !== "pes") {
      setError(`${ext.toUpperCase()} preview coming soon — quote still works below.`);
      return;
    }
    file.arrayBuffer().then((buf) => {
      try {
        setParsed(parseEmbroidery(buf, ext));
      } catch {
        setError(`Couldn't read this ${ext.toUpperCase()} file.`);
      }
    });
  }, [file]);
  ...
  return (
    <div className="space-y-4">
      <div className="relative h-[22rem] overflow-hidden rounded-3xl border border-border bg-slate-900 shadow-inner group">
        <div className="absolute inset-0 opacity-[0.03] pointer-events-none bg-[url('https://www.transparenttextures.com/patterns/fabric-plaid.png')]" />
        
        {parsed ? (
          <canvas ref={canvasRef} className="h-full w-full drop-shadow-[0_8px_24px_rgba(0,0,0,0.5)]" />
        ) : (
          <div className="grid h-full place-items-center p-8 text-center text-sm text-slate-400">
            {error ?? (
              <div className="flex flex-col items-center gap-3">
                <Sparkles className="h-8 w-8 text-pink-500/50 animate-pulse" />
      ...
      {parsed && (
        <div className="grid grid-cols-3 gap-3">
          <Stat icon={<Sparkles className="h-3.5 w-3.5" />} label="Total Stitches" value={parsed.stitchCount.toLocaleString()} />
          <Stat icon={<Ruler className="h-3.5 w-3.5" />} label="Actual Size" value={`${parsed.widthMm.toFixed(0)}×${parsed.heightMm.toFixed(0)} mm`} />
          <Stat icon={<Layers className="h-3.5 w-3.5" />} label="Thread Colors" value={`${parsed.colorChanges + 1}`} />
        </div>
      )}
    </div>
  );
