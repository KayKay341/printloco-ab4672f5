...
export const BUYER_STEPS = ["Account", "Service", "Upload", "Choose maker", "Checkout"];
...
export const OnboardingSteps = ({
  ...
  return (
    <div className="flex justify-between items-center mb-10 w-full max-w-2xl mx-auto px-4 gap-2">
      {steps.map((step, index) => {
        ...
        return (
          <div key={step} className="flex flex-col items-center gap-2 flex-1 min-w-0">
            ...
            <span className={`text-[11px] sm:text-xs font-semibold text-center truncate w-full ${isActive ? "text-foreground" : "text-muted-foreground"}`}>{step}</span>
          </div>
        );
      })}
    </div>
